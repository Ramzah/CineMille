...
public interface ProgrammazioneRepository extends CrudRepository<Programmazione, Integer>{
	...
	@Query(value = "SELECT sale.nome_sala, film.titolo, programmazione.data_orario FROM programmazione INNER JOIN film ON programmazione.id_film = film.id INNER JOIN sale ON programmazione.id_sala = sale.id;", nativeQuery = true)
